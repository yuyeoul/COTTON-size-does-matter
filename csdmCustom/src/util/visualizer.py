...
class Visualizer():
    def __init__(self, opt):
        ...
        self.eval_log_name = os.path.join(opt.checkpoint_dir, opt.name, 'eval_log.txt')
    ...
    table td {width: %dpx; height: %dpx; padding: 4px; outline: 4px solid black}
                ...
                idx = 1
                for label, image_numpy in visuals.items():
                    #print("inside the vis part ********")
                    #print("label = {}, image_numpy.shape = {}".format(label, image_numpy.shape))
                    #print("label = {}, type(image_numpy) = {}".format(label, type(image_numpy)))
                    #print(image_numpy)
